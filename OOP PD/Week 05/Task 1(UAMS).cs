...
namespace uams
{
    internal class Program
    {
        ...
        {
            int count = 0;
            SubjectDL.LoadSubjectDataFromFile();
	DegreeDL.LoadDegreeDataFromFile();
	StudentDL.LoadStudentDataFromFile();
            ...
            while (true)
            {
                ...
                if (option == 1)
                {
                    ...
                    if (DegreeDL.Degrees.Count > 0)
                        ...
                        StudentDL.AddStudent(StudentUI.Addstudent(DegreeDL.Degrees));
                    else
                    {
                ...
                {
                    foreach(Student s in StudentDL.Students)
	StudentDL.SaveStudentDataIntoFile(s);
                    foreach (Degree d in DegreeDL.Degrees)
	DegreeDL.SaveDegreeDataIntoFile(d);
                    break;
                }
        ...
        static void adddegreesubjects(List<Subject> subjects, List<Degree> degrees)
        {
            ...
            Subject sub = SubjectDL.AddSubjectToList(subjcode, subjtype, credhours, fees);
            subjects.Add(sub);
