...
namespace Task3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            loadrecords(Logins, ref s, ref i, ref c);
            ...
            while (true)
            {
                        ...
                        if (Logins[c].role == "Client" || Logins[c].role == "client")
                        {
                            ...
                            while (true)
                            {
                                ...
                                if (resc == "2")
                                {
                                    ...
                                    Console.Write("\n\n\n");
                                    if (Logins[c].depositmoney())
                                        Console.Write(" Your Amount is successfully deposited.");
                                }
                                if (resc == "3")
                                {
                                    Console.Clear();
                                    ...
                                    Console.Write("\n\n\n");
                                    if (Logins[c].withdrawmoney())
                                        Console.Write(" Your Amount is successfully withdraw.");
                                    else
                                ...
                                if (resc == "4")
                                {
                                    ...
                                    Console.Write("\n\n\n");
                                    if (Logins[c].transfermoney())
                                        Console.Write(" Your donations is successfully transfered.");
                                    else
                                        Console.Write(" Sorry! You don't have enough balance.");
                                }
                                if (resc == "5")
                        ...
                        else if (Logins[c].role == "Manager" || Logins[c].role == "manager")
                        {
                            ...
                            while (true)
                            {
                                ...
                                if (resm == "1")
                                {
                                    ...
                                    Console.Write("\n\n");
                                    if (viewaccounts(Logins, ref s, ref i, ref c))
                                ...
                                if (resm == "3")
                                {
                                    ...
                                    Console.Write("\n\n");
                                    if (deleteaccount(Logins, ref s, ref i, ref c))
                                        Console.Write(" Sorry! The client you want to delete does not exist.");
                                }
                                if (resm == "4")
                                {
                                    Console.Clear();
                                    viewtotal(Logins, ref s, ref i, ref c);
                                }
                                if (resm == "5")
                                {
                                    Console.Clear();
                ...
                else if (option == "3")
                {
                    ...
                    Console.Clear();
                    saverecordstofile(Logins, s, i, c);
                    break;
                }
            }
        }
        ...
        static void saverecordstofile(List<login> Logins, int s, int i, int c)
        {
            string filepath = "G:\\Task 05(Business Application)\\bankrecords.txt";
        ...
        static void loadrecords(List<login> Logins, ref int s, ref int i, ref int c)
        {
            i = 0;
            string record = "";
            string filepath = "G:\\Task 05(Business Application)\\bankrecords.txt";
            if (File.Exists(filepath))
            {
                using (StreamReader file = new StreamReader(filepath))
                {
                    ...
                    {
                        login u1 = new login("amir", 1234, "Client",0);
                        Logins.Add(u1);
                        ...
                        {
                            Logins[i].balance = int.Parse(getField(record, 4));
                        }
                        i = i + 1;
                    }
                }
        ...
        static bool viewaccounts(List<login> Logins, ref int s, ref int i, ref int c)
        {
            int count = 0;
            ...
            if (count != 0)
            {
                ...
                for (int p = 0; p < Logins.Count; p++)
                {
                    if ((Logins[p].role == "Client" || Logins[p].role == "client") && Logins[p].name != " ")
                        Console.WriteLine($" {Logins[p].name}\t\t{Logins[p].balance}");
                }
                return false;
            }
            else
        ...
        static void viewtotal(List<login> Logins, ref int s, ref int i, ref int c)
        {
            int totalb = 0;
            for (int n = 0; n < Logins.Count; n++)
            {
                totalb = totalb + Logins[n].balance;
            }
        ...
        static bool deleteaccount(List<login> Logins, ref int s, ref int i, ref int c)
        {
            int p = -1;
            string namen;
            string enter;
